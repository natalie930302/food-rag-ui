...
function Node({ x, y, w, h = 56, label, sub, state = "skip", badge }) {
  const c = C[state];
  return (
    <g>
      <rect x={x} y={y} width={w} height={h} rx={8} fill={c.fill} stroke={c.stroke} strokeWidth={1.2} strokeDasharray={c.dash} />
...
function FixedPath({ trace, meta }) {
  ...
  return (
    <svg viewBox="0 0 900 250" width="100%" role="img" aria-label="固定路徑的步驟圖">
      ...
      <Node x={120} y={20} w={95} label="拆子問題" sub={dec ? `${(dec.arguments?.subquestions || []).length} 個子問題` : "單一問題,不拆"} state={dec ? "done" : "skip"} />
      ...
      <Node x={576} y={24} w={96} h={52} label="信心閘門" sub={gateState === "skip" ? "≥ 0.52 才放行" : `${pct(bestRet)} ${anyRetOk ? "通過" : "未過"}`} state={gateState} />
      <Arrow d="M684 48 L696 48" on={retries.length > 0} />
      <Node x={700} y={20} w={95} label="改寫重查" sub={retries.length ? (anyRetryOk ? "改寫後通過" : "改寫後仍未過") : "閘門有過,不需要"} state={retryState} />
      {/* 換行連接 */}
      <path d="M 747 80 L 747 118 L 55 118 L 55 146" fill="none" stroke="#6366f1" strokeWidth={1.2} markerEnd="url(#pd-arrow-on)" />
      <Node x={10} y={150} w={90} label="查案例" sub={!cases ? "這題不需要" : cases.confident === false ? "相似度不足,不採用" : `${cases.chunk_ids?.length ?? 0} 筆,過門檻`} state={casesState} />
      <Arrow d="M104 178 L116 178" />
      <Node x={120} y={150} w={120} label={genState === "alt" ? "拒答" : "生成答案"} sub={genState === "alt" ? "法規與案例都不可信,不呼叫 LLM" : gen ? (meta?.confident === false ? "只憑案例作答" : "gpt-4o-mini 依段落作答") : ""} state={genState} />
      <Arrow d="M244 178 L256 178" on={!!gen} />
      <Node x={260} y={150} w={100} label="引用驗證" sub={ver ? (ver.detail?.includes("unsupported=0") ? "條號全部對得上" : "有條號對不上") : "拒答不需驗證"} state={ver ? (ver.detail?.includes("unsupported=0") ? "done" : "fail") : "skip"} />
...
// ---------- 審稿路徑 ----------
function ReviewPath({ trace, result }) {
  ...
  return (
    <svg viewBox="0 0 900 250" width="100%" role="img" aria-label="審稿路徑的步驟圖">
      ...
      <Node x={10} y={20} w={90} label="判定意圖" state="done" />
      <Arrow d="M104 48 L116 48" />
      <Node x={120} y={20} w={110} label="風險字掃描" sub={scan ? `${nKw} 個高風險詞` : ""} state={scan ? "done" : "skip"} />
      <Arrow d="M234 48 L246 48" />
      <Node x={250} y={20} w={150} label="檢索" sub="保證第 28 條 + 全庫補檢索" state={ret ? "done" : "skip"} />
      <Arrow d="M404 48 L416 48" />
      <Node x={420} y={20} w={100} label="查案例" sub={cases ? `${cases.chunk_ids?.length ?? 0} 筆相似廣告` : ""} state={cases ? "done" : "skip"} />
      <Arrow d="M524 48 L536 48" />
      <Node x={540} y={20} w={130} label="生成三段報告" sub="法規 / 案例 / 修改建議" state={gen ? "done" : "skip"} />
      <path d="M 605 80 L 605 118 L 55 118 L 55 146" fill="none" stroke="#6366f1" strokeWidth={1.2} markerEnd="url(#pd-arrow-on)" />
      <Node x={10} y={150} w={100} label="引用驗證" sub={ver ? (ver.detail?.includes("unsupported=0") ? "條號全部對得上" : "有條號對不上") : ""} state={ver ? (ver.detail?.includes("unsupported=0") ? "done" : "fail") : "skip"} />
      <Arrow d="M114 178 L126 178" />
      <Node x={130} y={150} w={100} label="判燈號" sub={verdictLabel} state={vd ? (result?.verdict === "high" ? "fail" : result?.verdict === "medium" ? "alt" : "done") : "skip"} />
...
function AgentPath({ trace, meta }) {
  ...
  return (
    <svg viewBox="0 0 900 300" width="100%" role="img" aria-label="agent 路徑的步驟圖">
      ...
      <Node x={136} y={36} w={308} h={70} label="工具:查法規(= 檢索管線 + 改寫重查)" sub={reg.length ? `${seq(order("tool:search_regulations"))}${regOk ? " · 有可信段落" : " · 都未過閘門"}${regFlags ? " · " + regFlags : ""}` : "沒呼叫"} state={reg.length ? (regOk ? "done" : "fail") : "skip"} badge={forced ? "強制" : undefined} />
      <Node x={136} y={120} w={308} label="工具:查案例" sub={cas.length ? `${seq(order("tool:search_violation_cases"))} · ${casOk ? "相似度過門檻,可當依據" : "相似度不足,不採用"}` : "沒呼叫"} state={cas.length ? (casOk ? "done" : "fail") : "skip"} />
      <Node x={136} y={190} w={308} label="工具:查關聯法條" sub={rel.length ? `${seq(order("tool:search_related_laws"))} · 條文共現統計(只當引用證據)` : "沒呼叫"} state={rel.length ? "done" : "skip"} />
...
export default function PipelineDiagram({ result }) {
  ...
  return (
    <div>
      {route.handler === "agent" ? <AgentPath trace={trace} meta={meta} />
        : route.handler === "review" ? <ReviewPath trace={trace} result={result} />
        : <FixedPath trace={trace} meta={meta} />}
      {legend}
    </div>
  );
}
